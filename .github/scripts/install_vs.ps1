...
if ($LASTEXITCODE -ne 0) {
    Write-Output "Download of the VS 2019 Version 16.8.5 installer failed"
    exit 1
}
...
if (Test-Path "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe") {
    ...
    if ($existingPath -ne $null) {
        Write-Output "Found existing BuildTools installation in $existingPath"
        ...
        if (($exitCode -ne 0) -and ($exitCode -ne 3010)) {
            Write-Output "Original BuildTools uninstall failed with code $exitCode"
            exit 1
        }
        Write-Output "Original BuildTools uninstalled"
    }
}
...
$exitCode = $process.ExitCode
if (($exitCode -ne 0) -and ($exitCode -ne 3010)) {
    Write-Output "VS installer exited with code $exitCode, which should be one of [0, 3010]."
    curl.exe --retry 3 -kL $COLLECT_DOWNLOAD_LINK --output Collect.exe
    if ($LASTEXITCODE -ne 0) {
        Write-Output "Download of the VS Collect tool failed."
        exit 1
    }
